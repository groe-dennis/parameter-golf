...
If you want a faster smoke test, you can validate on only a small prefix of the validation split by setting `VAL_SUBSET_FRACTION`, for example `VAL_SUBSET_FRACTION=0.01` to use about 1% of the validation sequences. Leaving it unset keeps the full validation split.
