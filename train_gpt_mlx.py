...
class Hyperparameters:
    ...
    # Validation uses the full fineweb_val split unless VAL_SUBSET_FRACTION is set.
    val_batch_size: int = int(os.environ.get("VAL_BATCH_SIZE", 524_288))
    val_subset_fraction: float = float(os.environ.get("VAL_SUBSET_FRACTION", 1.0))
    # When enabled, training keeps only the highest-loss half of tokens in each batch.
    loss_sampling_top_half: bool = bool(
        int(os.environ.get("LOSS_SAMPLING_TOP_HALF", "0"))
    )
    # When enabled, training keeps only the lowest-loss half of tokens in each batch.
    loss_sampling_bottom_half: bool = bool(
        int(os.environ.get("LOSS_SAMPLING_BOTTOM_HALF", "0"))
    )
...
def resolve_loss_sampling_mode(
...
def reduce_token_losses(losses: mx.array, sampling_mode: str | None) -> mx.array:
    flat_losses = losses.reshape(-1).astype(mx.float32)
    if sampling_mode is None:
        return mx.mean(flat_losses)

    n_tokens = int(flat_losses.shape[0])
    keep_tokens = max((n_tokens + 1) // 2, 1)
    if keep_tokens >= n_tokens:
        return mx.mean(flat_losses)
    sorted_losses = mx.sort(flat_losses)
    if sampling_mode == "top":
        selected = sorted_losses[-keep_tokens:]
    elif sampling_mode == "bottom":
        selected = sorted_losses[:keep_tokens]
    else:
        raise ValueError(f"Unknown loss sampling mode: {sampling_mode}")
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        qk_gain_init: float,
        loss_sampling_mode: str | None = None,
    ):
        super().__init__()
        ...
        self.loss_sampling_mode = loss_sampling_mode
    ...
    def loss(
        ...
        if sampling_mode is None:
            sampling_mode = self.loss_sampling_mode
        ...
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            ...
            losses = nn.losses.cross_entropy(
                logits.astype(mx.float32), y, reduction="none"
            )
            return reduce_token_losses(losses, sampling_mode)

        if sampling_mode is not None:
            loss_chunks: list[mx.array] = []
        else:
        ...
        for s in range(0, n, self.logit_chunk_tokens):
            ...
            if sampling_mode is None:
                ...
                losses = nn.losses.cross_entropy(
                    logits.astype(mx.float32), y[s:e], reduction="none"
                )
...
def select_validation_tokens(
    tokens: np.ndarray, seq_len: int, subset_fraction: float
    ...
    if not (0.0 < subset_fraction <= 1.0):
        raise ValueError(
            f"VAL_SUBSET_FRACTION must be in (0, 1], got {subset_fraction}"
        )
    subset_seqs = max(1, min(total_seqs, int(math.ceil(total_seqs * subset_fraction))))
    usable = subset_seqs * seq_len
    return tokens[: usable + 1], subset_seqs, total_seqs
...
def main() -> None:
    ...
    full_val_tokens = load_validation_tokens(args.val_files, args.train_seq_len)
    val_tokens, val_subset_seqs, total_val_seqs = select_validation_tokens(
        full_val_tokens,
        args.train_seq_len,
        args.val_subset_fraction,
    )
    ...
    # ==============================================================================
    loss_sampling_mode = resolve_loss_sampling_mode(
        args.loss_sampling_top_half, args.loss_sampling_bottom_half
    )
    model = GPT(
        ...
        loss_sampling_mode=loss_sampling_mode,
    )
    ...
    compiled_loss = mx.compile(
        lambda x, y: model.loss(x, y, sampling_mode=None),
        inputs=model.state,
        outputs=model.state,
    )
    compiled_loss_and_grad = mx.compile(
        nn.value_and_grad(
            model,
            lambda x, y: model.loss(x, y, sampling_mode=loss_sampling_mode),
        ),
        inputs=model.state,
        outputs=model.state,
    )
    ...
    if val_subset_seqs < total_val_seqs:
        log(
            f"val_loader:subset seqs:{val_subset_seqs}/{total_val_seqs} "
            f"fraction:{val_subset_seqs / total_val_seqs:.6f}"
        )
    ...
    log(f"loss_sampling_mode:{loss_sampling_mode or 'off'}")
    ...
    while True:
        ...
        if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
            ...
            # Validation scans the selected validation subset.
